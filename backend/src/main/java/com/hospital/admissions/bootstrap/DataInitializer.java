...
/**
 * Bootstrap component that populates the embedded H2 database with realistic
 * prototype clinical data.
 * <p>
 * Active only under the {@code prototype} profile. Seeds wards across multiple
 * levels,
 * diverse patient acuity profiles, pre-allocated inpatient admissions, pending
 * triage queues,
...
public class DataInitializer implements CommandLineRunner {
        ...
        public void run(String... args) {
                ...
                if (patientRepository.findByQueueToken("Q-P101").isEmpty()) {
                        seedEdPatients();
                }
        }
        ...
        private void seedWardsAndInpatients() {
                ...
                // ---- Ward 8B: Level 8 / Cardiology / Class B2 / Flex Unlocked (all
                // EMPTY_CLEANED for batch suggestions) ----
                ...
                for (int i = 1; i <= 4; i++) {
                        bedRepository.save(Bed.builder()
                                        ...
                                        .hasTelemetry(false)
                                        .cleaningStartedAt(i == 1 ? LocalDateTime.now().minusHours(3).minusMinutes(25)
                                                        : null)
                ...
                // ---- Ward 9B: Level 9 / General Medicine / Class B2 / Flex Unlocked (Dynamic
                // Cohort-Swap Scenario) ----
                ...
                // Approaching SLA turnover task (24 mins elapsed, 6 mins remaining, urgent
                // warning)
                bedRepository.save(Bed.builder()
                                .ward(ward10A)
                                .bedNumber("10A-03")
                ...
                // ---- Ward 11A: Level 11 / General Medicine / Class A / Negative Pressure
                // (MRSA/RESPIRATORY isolation) ----
        ...
        private void seedEdPatients() {
                ...
                // Scenario: bed-in-transit green state, check-in pending, negative-pressure
                // ward
                // ------------------------------------------------------------------
                ...
                admissionRequestRepository.save(AdmissionRequest.builder()
                                ...
                                .operationalDelayReason(
                                                "Ward 9A bed 9A-02 final sanitization delayed by EVS staffing gap. Estimated 20 mins.")
                ...
                // ------------------------------------------------------------------
                // P120 — Male, awaiting assessment (no admission request → on ED board).
                // Carries a synthesized clinical baseline with an elevated troponin,
                // so the ED smart-assessment form pre-populates Tier 2 Acute Urgent,
                // Cardiology, and Continuous Telemetry (Story 1.1.1 AC1 acute-cardiac path).
                // ------------------------------------------------------------------
                patientRepository.save(Patient.builder()
                                .name("Chua Wee Kiat")
                                .nricMasked("S****560X")
                ...
                // ------------------------------------------------------------------
                // P121 — Female, awaiting assessment (no admission request → on ED board).
                // Normal troponin with a completed baseline, so the smart-assessment
                // form defaults to the stable pathway (Tier 3, General Medicine, no
                // telemetry) — Story 1.1.1 stable/adjust-without-finalizing path.
                // ------------------------------------------------------------------
                patientRepository.save(Patient.builder()
                                .name("Nurul Huda")
                                .nricMasked("S****904Y")
                                ...
                                .needsTelemetry(false)
                                .queueToken("Q-P121")
                                .wardClassPreference(WardClass.C)
                                .suspectedDiagnosis("Community-acquired pneumonia, haemodynamically stable")
                                .vitalsBp("124/78")
                                .vitalsHr(84)
                                .vitalsSpo2(97)
                                .labTroponin("Normal")
                ...
                // Completes 3-patient surge cluster with P101 & P107 (Male, Class B2,
                // Non-infectious)
                ...
                // ------------------------------------------------------------------
                // P115 — Male, CARDIOLOGY, Tier 3, BED_ALLOCATED to Bed 9B-01 (waitingInEd =
                // true)
                ...
                // ------------------------------------------------------------------
                // P116 — Male, GENERAL_MEDICINE, Tier 4 Subacute Post-Stroke, Sister Hospital
                // Referral
                // Scenario: active bilateral 30-minute SLA countdown to Outram Community
                // Hospital (OCH)
                ...
                // ------------------------------------------------------------------
                // P118 — Female, GENERAL_MEDICINE, Tier 2 Acute Urgent, Prolonged Wait (>60m
                // SLA)
                // Scenario: SPECIALIZED_ISOLATION_CLEANING delay tag + prolonged wait visual
                // alert
                ...
                admissionRequestRepository.save(AdmissionRequest.builder()
                                ...
                                .operationalDelayReason(
                                                "Specialized isolation room completing mandatory 30-minute UV disinfection cycle for patient safety. Preparing room.")
