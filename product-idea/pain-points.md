...
| **4** | **"Ghost Capacity" from Multi-Bed Cubicle Gender/Pathogen Locking** | In Class B2/C wards, 1 patient's gender or infection status locks all remaining beds in a 4- to 6-bed cubicle. Ad-hoc allocation causes empty beds that cannot be used. | **Four-State (Mustard Yellow/White/Green/Grey) Bed Orchestration & Dynamic Holding Room Batching:** Implements a strict bed state machine (`Mustard Yellow` = vacated/empty/pending cleaning, `White` = empty/clean, `Green` = assigned/in-transit, `Grey` = occupied). Algorithm runs a two-phase strategy: Phase 1 packs patients into existing partially filled cubicles matching their Ward Class (C, B2, B1, A), gender, and infection status. Phase 2 clusters remaining waiting patients and proactively recommends designating all-White cubicles into dedicated 'Holding Rooms' for batch admission. Includes dynamic 'Cohort-Swap' re-optimizations to reassign isolated Green patients before ED departure, freeing entire cubicles for large surge cohorts. |
...
- **Four-State Bed Lifecycle Machine (`Mustard Yellow` / `White` / `Green` / `Grey`):**
  - **`Mustard Yellow` (Vacated, Empty & Pending Cleaning):** Patient has been discharged, and the bed is vacated and empty, but not yet cleaned (30-min housekeeping sanitization in progress; ineligible for allocation).
  - **`White` (Available & Clean):** Vacant, sanitized by housekeeping, and immediately available for allocation ("empty, cleaned").
  ...
  - *Turnover Transition Gate:* When a patient is discharged, the bed turns **`Mustard Yellow`** (vacated, empty, not yet cleaned) and remains unavailable during the 30-minute housekeeping SLA, turning **`White`** (empty, cleaned) only upon terminal cleaning sign-off $\rightarrow$ **`Green`** upon BMU allocation $\rightarrow$ **`Grey`** upon physical ward reception.
