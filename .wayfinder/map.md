...
- **Prototype Strategy vs Target Production Architecture**:
  - *Spring Profile Boundary (`prototype` vs `prod`)*: Prototype decisions (in-memory H2, synthetic `DataInitializer` seeds, and mocked external gateways) are strictly gated under `@Profile("prototype")` (`spring.profiles.active=prototype`). In `prod`, Spring Dependency Injection swaps these for real persistent databases and actual external HTTP/FHIR adapters without altering domain services.
