...
class BmuControllerTest {
    ...
    void testOverrideAllocation() throws Exception {
        ...
        AdmissionRequest allocated = AdmissionRequest.builder().id(reqId).overrideReasonCode("GOVERNMENT_SUBSIDY_CLASS_UPGRADE").build();
        when(bmuService.allocateBed(reqId, bedId, 2, 70.0, "GOVERNMENT_SUBSIDY_CLASS_UPGRADE")).thenReturn(allocated);
    ...
    void testAllocateBed_SafetyInvariant_Returns422() throws Exception {
        ...
        when(bmuService.allocateBed(reqId, bedId))
                .thenThrow(new com.hospital.admissions.exception.SafetyInvariantViolationException("Biological gender cohorting violation in multi-bed ward"));
    ...
    void testAllocateBed_OperationalConstraintWithoutReason_Returns400() throws Exception {
        ...
        when(bmuService.allocateBed(reqId, bedId))
                .thenThrow(new IllegalArgumentException("Operational constraint override requires a valid structured reason code"));
    ...
    void testDeallocateBed() throws Exception {
        UUID reqId = UUID.randomUUID();
        com.hospital.admissions.dto.BedDeallocationRequest deallocReq =
                new com.hospital.admissions.dto.BedDeallocationRequest(reqId);

        AdmissionRequest deallocated = AdmissionRequest.builder().id(reqId).status(com.hospital.admissions.domain.AdmissionStatus.BED_REQUESTED).build();
    ...
    void testGetBatchSuggestions() throws Exception {
        ...
        com.hospital.admissions.dto.BatchSuggestion suggestion = com.hospital.admissions.dto.BatchSuggestion.builder()
                .suggestionId("SUGG-1")
                .targetWardId(wardId)
    ...
    void testApproveBatchHoldingWard() throws Exception {
        ...
        AdmissionRequest r1 = AdmissionRequest.builder().id(reqId1).status(com.hospital.admissions.domain.AdmissionStatus.BED_ALLOCATED).build();
        AdmissionRequest r2 = AdmissionRequest.builder().id(reqId2).status(com.hospital.admissions.domain.AdmissionStatus.BED_ALLOCATED).build();
        AdmissionRequest r3 = AdmissionRequest.builder().id(reqId3).status(com.hospital.admissions.domain.AdmissionStatus.BED_ALLOCATED).build();
    ...
    void testGetCohortSwapSuggestions() throws Exception {
        UUID reqId = UUID.randomUUID();
        com.hospital.admissions.dto.CohortSwapSuggestion suggestion = com.hospital.admissions.dto.CohortSwapSuggestion.builder()
                .suggestionId("SWAP-1")
                .admissionRequestId(reqId)
    ...
    void testApproveCohortSwap() throws Exception {
        ...
        AdmissionRequest result = AdmissionRequest.builder().id(reqId).status(com.hospital.admissions.domain.AdmissionStatus.BED_ALLOCATED).build();
        when(bmuService.approveCohortSwap(any(com.hospital.admissions.dto.CohortSwapApprovalRequest.class))).thenReturn(result);
    ...
    void testRecallDiversion() throws Exception {
        UUID reqId = UUID.randomUUID();
        AdmissionRequest result = AdmissionRequest.builder().id(reqId).status(com.hospital.admissions.domain.AdmissionStatus.BED_REQUESTED).build();
    ...
    void testLogTelephoneFollowUp() throws Exception {
        UUID reqId = UUID.randomUUID();
        com.hospital.admissions.dto.FollowUpNoteRequest noteReq = new com.hospital.admissions.dto.FollowUpNoteRequest("Spoke with partner hospital");
        AdmissionRequest result = AdmissionRequest.builder().id(reqId).operationalDelayReason("Spoke with partner hospital").build();
    ...
    void testAttachDelayTag_Forbidden_WhenNonBmu() throws Exception {
        ...
        when(bmuService.attachDelayTag(eq(reqId), any(DelayTagRequest.class)))
                .thenThrow(new org.springframework.security.access.AccessDeniedException("Only BMU coordinators possess operational authority to tag delay reasons."));
