  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-center py-4">
        <div className="w-full max-w-sm rounded-[2.5rem] border-[8px] border-slate-800 bg-slate-900 p-2 shadow-2xl">
          {/* Top Speaker Notch */}
          <div className="relative mx-auto mb-2 h-4 w-28 rounded-full bg-slate-800 flex items-center justify-center">
            <div className="h-1.5 w-1.5 rounded-full bg-slate-700 mr-2"></div>
            <div className="h-1 w-10 rounded-full bg-slate-700"></div>
          </div>

          {/* Screen Container */}
          <div className="rounded-[2rem] bg-slate-50 overflow-hidden min-h-[580px] flex flex-col justify-between p-4 text-slate-900">
            {/* Header in App */}
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-sm text-slate-900 tracking-tight">SingHealth HealthHub</span>
              ...
              {isLoading ? (
                <div className="py-16 text-center text-xs text-slate-500">Loading tracking status...</div>
              ) : error ? (
                <div className="py-16 text-center text-xs text-red-600">Failed to track token.</div>
              ) : tracker ? (
                <>
                  {/* Greeting & Summary */}
                  <div className="bg-emerald-600 text-white p-3.5 rounded-xl shadow-xs space-y-1">
                    ...
                    <p className="text-xs text-emerald-100">{tracker.milestoneLabel}</p>
                  </div>
                  ...
                  {tracker.assignedBedNumber && (
                    <div className="bg-blue-50 border border-blue-200 p-2.5 rounded-lg text-xs flex items-center justify-between text-blue-900">
                      <div className="flex items-center gap-2">
                        <BedDouble className="h-4 w-4 text-blue-600" />
                        <div>
                          <div className="font-bold">Bed {tracker.assignedBedNumber}</div>
                          <div className="text-[10px] text-blue-700">Ward {tracker.assignedWardCode}</div>
                        </div>
                      </div>
                      <Badge variant="outline" className="text-[10px] bg-white text-blue-700 font-medium">
                        Assigned
                      </Badge>
                    </div>
                  )}
                  ...
                  {tracker.delayReason && (
                    <div className="bg-amber-50 border border-amber-200 p-2 rounded-lg text-[11px] text-amber-900 flex items-start gap-1.5">
                  ...
                  <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-xs space-y-2.5">
                    ...
                    <div className="space-y-3">
                      {MILESTONES.map((m) => {
                        ...
                        return (
                          <div key={m.step} className="flex items-start gap-2.5">
                            <div
                              className={`h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5 flex-shrink-0 ${
                                isPast
                            ...
                            >
                              {isPast ? <CheckCircle2 className="h-3 w-3" /> : m.step}
                            </div>
                            <div className="text-xs">
                              <div
                                className={`font-semibold ${
                                  isCurrent ? 'text-blue-600 font-bold' : isPast ? 'text-slate-900' : 'text-slate-400'
                                }`}
                              >
                                {m.title}
                              </div>
                  ...
                  {tracker.financialExplainer && (
                    <div className="bg-slate-100 p-3 rounded-xl text-xs space-y-1.5">
                      <div className="flex items-center justify-between text-[11px] font-bold text-slate-800">
                        ...
                        <span className="text-[10px] text-slate-500 font-normal">FYI Only</span>
                      </div>
                      ...
                      <div className="pt-1 flex items-center justify-between text-[10px] text-blue-700 font-medium">
                        <span className="flex items-center gap-1">
                          <PhoneCall className="h-3 w-3" /> Medical Social Work:
                        </span>
                        <span>{tracker.financialExplainer.mswContact}</span>
                      </div>
                    </div>
                  )}
                </>
