...
The project is built as a Spring Boot 4.1 + React 19 (TypeScript) monorepo. I designed the architecture to be production-ready by default — when no Spring profile is active, default beans connect to real databases and external APIs. All prototype shortcuts and simplifications are strictly isolated under `@Profile("prototype")`.
...
flowchart TD
    ...
    subgraph Solvers
        ...
        D -.-> |"Default (Production)"| F[TimefoldSolver]
    end
    ...
    K -.-> |"Default (Production)"| M[FhirHospitalEhrGateway]

    N(SisterHospitalGateway) -.-> |"@Profile('prototype')"| O[MockSisterHospitalGateway]
    N -.-> |"Default (Production)"| P[HttpSisterHospitalGateway]
```
...
| Dimension | Prototype Approach | Production Target | Design Rationale | Relevant Ticket |
| ----------- | ------------------- | ------------------- | ------------------ | ----------------- |
...
- **Gateway / Adapter Pattern (Hexagonal Architecture)**: Isolates core business logic from external systems (EHR, sister hospitals).
- **Spring Profile Boundary Pattern**: All prototype functionality is explicitly isolated using `@Profile("prototype")`.
- **Pluggable Strategy Pattern**: The `BedAllocationSolver` interface allows swapping between heuristic and Timefold implementations.
- **Gated State Machine**: Distinguishes between Consult-Gated and Direct Admission flows.
- **Consensus Completion Gate Pattern**: Manages multi-department agreement before state transitions.
- **Safety-First Discordance Engine**: Ensures patient safety by applying highest acuity and telemetry union rules during conflict resolution.
- **Two-Tier Constraint Hierarchy**: Separates Absolute Safety Invariants (hard constraints) from Overridable Operational Constraints (soft constraints).
- **Dual-Pathway Metric Extraction**: Extracts metrics reliably via SQL or Structured Logs.
- **Auditable Entity Pattern with MDC**: Ensures all state changes are contextually logged for compliance.
